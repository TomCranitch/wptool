...
object SMT {
  ...
  def prove(cond: Expression, given: List[Expression], debug: Boolean, expectIds: Boolean = false) = {
    ...
    if (debug) {
      ...
      if (res == z3.Status.SATISFIABLE) {
        ...
        println("Model: [" + cond.vars.toList.sortWith((x, y) => x.toString < y.toString).map(x => x + " -> " + model.eval(translate(x, expectIds), false)).mkString(", ") + "]")
      }
  ...
  def formula(prop: Expression, expectIds: Boolean = false): z3.BoolExpr = translate(prop, expectIds) match {
    case b: z3.BoolExpr => b
    case e =>
      throw error.InvalidProgram("not a boolean expression", prop, e)
  }

  def arith(prop: Expression, expectIds: Boolean = false): z3.IntExpr = translate(prop, expectIds) match {
  ...
  def bitwise(prop: Expression, expectIds: Boolean = false): z3.BitVecExpr = translate(prop, expectIds) match {
  ...
  def handleStore (store: Expression, arr: z3.ArrayExpr, expectIds: Boolean): z3.Expr = store match {
    case a: VarAccess => ctx.mkSelect(arr, translate(a.index, expectIds))
    case a: VarStore => handleStore(a.array, ctx.mkStore(arr, translate(a.index, expectIds), translate(a.exp, expectIds)), expectIds)
  ...
  def translate(prop: Expression, expectIds: Boolean): z3.Expr = prop match {
    ...
    case x: VarAccess =>  
      ...
      ctx.mkSelect(ctx.mkArrayConst(x.name.toString, ctx.getIntSort, sort), translate(x.index, expectIds))
    case x: IdAccess =>  
      if (!expectIds) throw new Error("unresolved array id")
      val sort = if (x.ident.gamma) ctx.getBoolSort else ctx.getIntSort
      ctx.mkSelect(ctx.mkArrayConst(x.ident.toString, ctx.getIntSort, sort), translate(x.index, expectIds))

    case store: VarStore => handleStore(store, ctx.mkArrayConst(store.name.toString, ctx.getIntSort, if (store.isBool) ctx.getBoolSort else ctx.getIntSort), expectIds)
    ...
    case const: ArrayConstDefault => 
      ...
      if (const.name.ident.gamma) ctx.mkEq(ctx.mkArrayConst(const.name.toString, ctx.getIntSort, ctx.getBoolSort), ctx.mkConstArray(ctx.getIntSort, translate(const.const, expectIds)))
      else throw new Error("ArrayConstDefault is only for gamma values")
      

    case BinOp("==", arg1, arg2) => ctx.mkEq(translate(arg1, expectIds), translate(arg2, expectIds))
    case BinOp("!=", arg1, arg2) => ctx.mkNot(ctx.mkEq(translate(arg1, expectIds), translate(arg2, expectIds)))

    case PreOp("!", arg) => ctx.mkNot(formula(arg, expectIds))
    case BinOp("&&", arg1, arg2) => ctx.mkAnd(formula(arg1, expectIds), formula(arg2, expectIds))
    case BinOp("||", arg1, arg2) => ctx.mkOr(formula(arg1, expectIds), formula(arg2, expectIds))

    case BinOp("=>", arg1, arg2) => ctx.mkImplies(formula(arg1, expectIds), formula(arg2, expectIds))

    case PreOp("-", arg) => ctx.mkUnaryMinus(arith(arg))
    case BinOp("+", arg1, arg2) => ctx.mkAdd(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp("-", arg1, arg2) => ctx.mkSub(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp("*", arg1, arg2) => ctx.mkMul(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp("/", arg1, arg2) => ctx.mkDiv(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp("%", arg1, arg2) => ctx.mkMod(arith(arg1, expectIds), arith(arg2, expectIds))

    case BinOp("<=", arg1, arg2) => ctx.mkLe(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp("<", arg1, arg2) => ctx.mkLt(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp(">=", arg1, arg2) => ctx.mkGe(arith(arg1, expectIds), arith(arg2, expectIds))
    case BinOp(">", arg1, arg2) => ctx.mkGt(arith(arg1, expectIds), arith(arg2, expectIds))

    case BinOp("|", arg1, arg2) => ctx.mkBVOR(bitwise(arg1, expectIds), bitwise(arg2, expectIds))
    case BinOp("&", arg1, arg2) => ctx.mkBVAND(bitwise(arg1, expectIds), bitwise(arg2, expectIds))
    case BinOp("^", arg1, arg2) => ctx.mkBVXOR(bitwise(arg1, expectIds), bitwise(arg2, expectIds))
    case PreOp("~", arg) => ctx.mkBVNot(bitwise(arg))

      // defining normal right shift as logical shift right
    case BinOp(">>", arg1, arg2) => ctx.mkBVLSHR(bitwise(arg1, expectIds), bitwise(arg2, expectIds))
    case BinOp(">>>", arg1, arg2) => ctx.mkBVASHR(bitwise(arg1, expectIds), bitwise(arg2, expectIds))
    case BinOp("<<", arg1, arg2) => ctx.mkBVSHL(bitwise(arg1, expectIds), bitwise(arg2, expectIds))

    case Question(test, arg1, arg2) => ctx.mkITE(formula(test, expectIds), translate(arg1, expectIds), translate(arg2, expectIds))


    // boundConstraints, body, 0, scala.Arry(), null, null, null
    case ForAll(bound, body) =>
      ctx.mkForall(bound.toArray.map(b => translate(b, expectIds)), translate(body, expectIds), 0, scala.Array(), null, null, null)
