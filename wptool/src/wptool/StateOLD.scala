...
case class StateOLD(
  ...
  def assign(id: Id, arg: Expression): StateOLD = {
    val v = id.toVar
    ...
    val PPrimeRestrict = StateOLD.restrictP(PPrime, stable)
  ...
  def assignCAS(lhs: Id, x: Id, r1: Expression, r2: Expression): StateOLD = {
    val _lhs = lhs.toVar
    val _x = x.toVar
  ...
  def arrayAssign(a: Id, index: Expression, arg: Expression, possible: Seq[Int]): StateOLD = {
    val v = a.toVar
    // create mapping from variable to fresh variable
    ...
    val POut = if (possible.size == 1) {
      ...
      // restrict PPrime to stable variables
      StateOLD.restrictP(PPrime, stable)
    } else {
  ...
  def addToP(expr: Expression): StateOLD = {
    copy(P = expr :: P)
  }

  def resetReadWrite(): StateOLD = {
    copy(read = Set(), written = Set())
  }

  def updateRead(id: Id): StateOLD = {
    if (debug)
      println("updating read (" + read + ") with " + id)
    copy(read = read + id)
  }

  def updateRead(id: Set[Id]): StateOLD = {
    if (debug)
      println("updating read (" + read + ") with " + id)
    copy(read = read ++ id)
  }

  def updateWritten(id: Id): StateOLD = {
    if (debug)
      println("updating written (" + written + ") with " + id)
    copy(written = written + id)
  }

  def updateWritten(id: Set[Id]): StateOLD = {
    if (debug)
      println("updating written (" + written + ") with " + id)
    copy(written = written ++ id)
  }

  def updateWritten(array: IdArray): StateOLD = {
  ...
  def updateRead(array: IdArray): StateOLD = {
    val ids: Set[Id] = {for (i <- array.array.indices)
      yield array.array(i)}.toSet
    if (debug)
  ...
  def updateDAssign(x: Id, e: Expression) : StateOLD = {
  ...
  def updateDArrayAssign(x: Id, e: Expression) : StateOLD = {
    val varE = e.variables // var(e)
  ...
  def updateDCAS(r3: Id, x: Id, r1: Expression, r2: Expression) : StateOLD = {
  ...
  def updateDGuard(b: Expression) : StateOLD = {
  ...
  def updateGamma(x: Id, t: Security): StateOLD = {
    if (gamma.contains(x)) {
  ...
  def updateGammaArray(a: IdArray, indices: Seq[Int], t: Security): StateOLD = {
  ...
  def updateGammaCAS(x: Id, t: Security): StateOLD = {
    if (gamma.contains(x)) {
  ...
  def updateGammaDomain(x: Id, t: Security): StateOLD = {
    val gammaPrime = gamma + (x -> t)
    copy(gamma = gammaPrime)
  }

  def updateGammasDomain(sec: Map[Id, Security]): StateOLD = {
    val gammaPrime = gamma ++ sec
    copy(gamma = gammaPrime)
  }

  def removeGamma(xs: Set[Id]): StateOLD = {
    val gammaPrime = gamma -- xs
    copy(gamma = gammaPrime)
  }

  def removeGamma(x: Id): StateOLD = {
  ...
  def restrictP(restricted: Set[Id]): List[Expression] = {
    ...
    StateOLD.restrictP(P, restricted ++ locals)
  }

  def updatePIfLeft(b: Expression): StateOLD = {
  ...
  def updatePIfRight(b: Expression): StateOLD = {
    // negate b
    val notB = PreOp("!", b)
    // remove free occurrences of unstable variables
  ...
  def mergeIf(state2: StateOLD): StateOLD = {
    val state1 = this
    ...
    val PPrimeRestricted = StateOLD.restrictP(PPrime, stable)
    if (debug)
  ...
  // D' = D1 intersect D2
  def mergeD(state2: StateOLD): Map[Id, (Set[Id], Set[Id], Set[Id], Set[Id])] = {
    StateOLD.mergeD(this.D, state2.D)
  }
...
*/
  ...
  def setMode(z: Id, mode: Mode): StateOLD = {
    mode match {
      case NoW =>
  ...
  def setModes(ids: Set[Id], mode: Mode): StateOLD = {
    mode match {
      case NoW =>
  ...
  def setModes(modes: Map[Id, Mode]): StateOLD = {
...
object StateOLD {
  def init(definitions: Set[Definition], P_0: Option[List[Expression]], gamma_0: Option[List[GammaMapping]],
           toLog: Boolean, debug: Boolean, noInfeasible: Boolean): StateOLD = {
