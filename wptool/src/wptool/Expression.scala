...
case class Id(name: String, override val expType: Type, prime: Boolean, gamma: Boolean, nought: Boolean, memLoc: Boolean)
    extends Identifier {
  ...
  // TODO change type
  def toGamma = Id(name, TBool, prime, true, nought, false)

  def getIndex(state: State) = {
    if (!gamma) state.indicies.getOrElse(this.copy(memLoc = false), throw new Error(s"Index not found for var $this with type $expType"))
    else
  ...
  override def ident = this

  // TODO use actual type
  override def getBase = Id(name, TInt, false, false, false, false)
}

object Id {
  val tmpId = Id("tmp", TInt, false, false, false, false)
  val indexId = Id("_i", TInt, false, false, false, false)
  val memId = Id("mem", TInt, false, false, false, false)

  def getAddr(id: Id, state: State): Expression = {
    state.addrs.getOrElse(id.getBase, throw new Error("Couldn't resolve memeory address"))
  }
}
...
case class IdAccess(ident: Id, index: Expression) extends Expression with Identifier {
  def this(name: String, index: Expression) = this(Id(name, TInt, false, false, false, false), index)
  def this(name: String, prime: Boolean, gamma: Boolean, index: Expression) = this(Id(name, TInt, prime, gamma, false, false), index)
      ...
      su._1
        .filter {
          ...
          case (d @ Dereference(v: Var), _) =>
            !su._2.arrayIds.contains(
              ...
              .contains(v.ident.getBase)
          case (v: ObjVarAccess, _) =>
            !su._2.arrayIds.contains(
              v.ident.getBase
            ) && this.ident.gamma == v.ident.gamma && this.ident.prime == v.ident.prime && this.ident.nought == v.ident.nought && su._2.globals
              .contains(v.ident.getBase)
          case s @ _ => throw new Error(s"Unexpected subst ($s, ${s._1.getClass})")
        }
        .foldLeft(memId: Expression) {
          ...
          case (p, (o: ObjVarAccess, Left(e))) =>
            this.index match {
              case _ if (name.index != o.name.index) => p
              case _                                 => VarStore(p, o.getAddr(su._2), e)
            }
        }
...
case class ObjIdAccess(ident: Id, field: String) extends Expression with Identifier {
  def this(name: String, field: String) = this(Id(name, TInt, false, false, false, false), field)
  ...
  def toPrime = this.copy(ident = ident.toPrime)
  override def expType = ident.expType
  def getAddr(state: State) = BinOp("+", TInt, TInt, state.addrs.get(ident.getBase).get, Lit(0)) // TODO
...
case class ObjVarAccess(name: Var, field: String) extends Expression with Variable {
  ...
  def subst(su: Subst) =
    if (su._1.contains(this)) su._1.get(this).get.left.get
    else this
  override def toString = ident + "." + field
  def toGamma(state: State) = this.copy(name = name.toGamma(state))
  def toPrime(state: State) = this.copy(name = name.toPrime(state))
  ...
  def getAddr(state: State) = ObjIdAccess(name.ident, field).getAddr(state)
...
case class CompareAndSwap(x: Id, e1: Expression, e2: Expression) extends Expression {
  def this(x: String, e1: Expression, e2: Expression) = this(new Id(x, TInt, false, false, false, false), e1, e2)
...
case class Dereference(ident: Expression) extends Expression {
  def this(x: String) = this(Id(x, TInt, false, false, false, false))
